...
class Attachments extends _$Attachments {
  @override
  List<NcAttachedFile> build(String? rowId, String columnTitle) {
  ...
  Future<void> rename(String id, String title, FnOnUpdate onUpdate) async {
    state = [...state]
        .map<NcAttachedFile>((e) => e.id == id ? e.copyWith(title: title) : e)
        .toList();
    await onUpdate({columnTitle: state});
  }
}
